...
def main():
    st.title('Data Berdasarkan Season dan Tahun')

    # Memuat data Anda
    data = pd.read_csv('hour.csv')

    # Pilihan season dan tahun
    seasons = ['Spring', 'Summer', 'Fall', 'Winter']
    years = [2011, 2012]

    # Widget selectbox untuk season dan tahun
    selected_season = st.selectbox('Pilih Season:', seasons)
    selected_year = st.selectbox('Pilih Tahun:', years)

    # Memfilter data berdasarkan season dan tahun yang dipilih
    filtered_data = data[(data['season'] == selected_season) & (data['Year'] == selected_year)]

    # Menampilkan hasil
    st.write(f"Hasil untuk Season: {selected_season} Tahun: {selected_year}")
